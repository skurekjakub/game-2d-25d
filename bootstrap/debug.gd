...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	match event.keycode:
		...
		KEY_F5:
			log_damage = not log_damage
			print("[Debug] log_damage = %s" % log_damage)
		KEY_F12:
			show_overlay = not show_overlay
			print("[Debug] show_overlay = %s" % show_overlay)
		# F1/F2/F6/F7 added in milestone 1.7 when their targets exist
...
func _on_damage_dealt(source: Node, target: Node, amount: float) -> void:
	if not log_damage:
		return
	_log(
		"damage_dealt(source=%s, target=%s, amount=%.1f)" % [_label(source), _label(target), amount]
	)
