> **Source:** [https://gameprogrammingpatterns.com/flyweight.html](https://gameprogrammingpatterns.com/flyweight.html)
...
## <a href="#forest-for-the-trees" id="forest-for-the-trees">Forest for the Trees</a>
...
The key observation is that even though there may be thousands of trees in the forest, they mostly look similar. They will likely all use the <span id="same">same</span> mesh and textures. That means most of the fields in these objects are the *same* between all of those instances.

You’d have to be crazy or a billionaire to budget for the artists to individually model each tree in an entire forest.

<span id="trees"></span>

![A row of trees, each of which has its own Mesh, Bark, Leaves, Params, and Position.](images/flyweight-trees.png)

Note that the stuff in the small boxes is the same for each tree.

We can model that explicitly by splitting the object in half. First, we pull out the data that all trees have <span id="type">in common</span> and move it into a separate class:
...
![A row of trees each with its own Params and Position, but pointing to a shared Model with a Mesh, Bark, and Leaves.](images/flyweight-tree-model.png)
...
## <a href="#a-thousand-instances" id="a-thousand-instances">A Thousand Instances</a>
...
Fortunately, today’s graphics APIs and <span id="hardware">cards</span> support exactly that. The details are fiddly and out of the scope of this book, but both Direct3D and OpenGL can do something called [*instanced rendering*](http://en.wikipedia.org/wiki/Geometry_instancing).
...
## <a href="#the-flyweight-pattern" id="the-flyweight-pattern">The Flyweight Pattern</a>
...
## <a href="#a-place-to-put-down-roots" id="a-place-to-put-down-roots">A Place To Put Down Roots</a>
...
Because we game programmers are paranoid about efficiency, there’s no way we’d store all of that state in <span id="learned">each</span> tile in the world. Instead, a common approach is to use an enum for terrain types:
...
![A row of tiles. Each tile points to either a shared Grass, River, or Hill object.](images/flyweight-tiles.png)
...
[Next<span class="full-nav"> Chapter</span>](observer.html) →
